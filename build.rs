...
fn mm_version() -> String {
    ...
    if !v_file.is_empty() {
        if !v_file.contains(&version) {
            ...
            // If the file contains the latest commit hash then this is either a CI build and the version generated by CI should be written to env
            // or this is a local build with no change in commits so it's alright to write the version from the file to the env
            version = v_file;
        }
    }
