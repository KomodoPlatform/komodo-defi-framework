...
mod sql_tx_history_storage_tests {
    ...
    fn test_add_and_get_tx_from_cache() {
        ...
        block_on(storage.add_tx_to_cache(for_coin, &BytesJson(tx.tx_hash.clone().into_bytes()), &tx.tx_hex)).unwrap();

        let tx_hex = block_on(storage.tx_bytes_from_cache(for_coin, &BytesJson(tx.tx_hash.clone().into_bytes())))
            .unwrap()
            .unwrap();
