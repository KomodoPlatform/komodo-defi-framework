...
fn nft_list_builder_preimage(
    ...
    let mut sql_builder = SqlBuilder::select_from(table_name.0);
...
fn select_last_block_number_sql(table_name: SafeTableName) -> Result<String, SqlError> {
    let sql = format!(
...
fn select_last_scanned_block_sql() -> MmResult<String, SqlError> {
    let table_name = scanned_nft_blocks_table_name()?;
    let sql = format!("SELECT last_scanned_block FROM {} WHERE chain=?1", table_name.0,);
    Ok(sql)
}

fn delete_nft_sql(table_name: SafeTableName) -> Result<String, SqlError> {
    let sql = format!("DELETE FROM {} WHERE token_address=?1 AND token_id=?2", table_name.0);
    Ok(sql)
}
...
fn get_nfts_by_token_address_statement(conn: &Connection, table_name: SafeTableName) -> Result<Statement, SqlError> {
    let sql_query = format!("SELECT * FROM {} WHERE token_address = ?", table_name.0);
    let stmt = conn.prepare(&sql_query)?;
    Ok(stmt)
}

fn get_token_addresses_statement(conn: &Connection, table_name: SafeTableName) -> Result<Statement, SqlError> {
    let sql_query = format!("SELECT DISTINCT token_address FROM {}", table_name.0);
    let stmt = conn.prepare(&sql_query)?;
    Ok(stmt)
}
...
fn get_transfers_with_empty_meta_builder<'a>(conn: &'a Connection, chain: &'a Chain) -> Result<SqlQuery<'a>, SqlError> {
    let table_name = chain.transfer_history_table_name()?;
    let mut sql_builder = SqlQuery::select_from(conn, table_name.0.as_str())?;
    sql_builder
        .sql_builder()
        .distinct()
        .field("token_address")
...
fn is_table_empty(conn: &Connection, table_name: SafeTableName) -> Result<bool, SqlError> {
    let query = format!("SELECT COUNT(*) FROM {}", table_name.0);
...
impl NftListStorageOps for AsyncMutexGuard<'_, AsyncConnection> {
    ...
    async fn is_initialized(&self, chain: &Chain) -> MmResult<bool, Self::Error> {
        let table_name = chain.nft_list_table_name()?;
        self.call(move |conn| {
            let nft_list_initialized = query_single_row(conn, CHECK_TABLE_EXISTS_SQL, [table_name.0], string_from_row)?;
            let scanned_nft_blocks_initialized = query_single_row(
                conn,
                CHECK_TABLE_EXISTS_SQL,
                [scanned_nft_blocks_table_name()?.0],
                string_from_row,
            )?;
    ...
    async fn get_nft(
        ...
        let table_name = chain.nft_list_table_name()?;
        self.call(move |conn| {
            let sql = format!("SELECT * FROM {} WHERE token_address=?1 AND token_id=?2", table_name.0);
    ...
    async fn clear_nft_data(&self, chain: &Chain) -> MmResult<(), Self::Error> {
        ...
        let sql_nft = format!("DROP TABLE IF EXISTS {};", table_nft_name.0);
        let table_scanned_blocks_name = scanned_nft_blocks_table_name()?;
        let sql_scanned_block = format!("DELETE from {} where chain=?1", table_scanned_blocks_name.0);
        ...
        self.call(move |conn| {
            ...
            sql_transaction.commit()?;
            if is_table_empty(conn, table_scanned_blocks_name.clone())? {
                conn.execute(&format!("DROP TABLE IF EXISTS {};", table_scanned_blocks_name.0), [])
                    .map(|_| ())?;
            }
            Ok(())
        })
    ...
    async fn clear_all_nft_data(&self) -> MmResult<(), Self::Error> {
        self.call(move |conn| {
            ...
            for chain in Chain::variant_list().into_iter() {
                let table_name = chain.nft_list_table_name()?;
                sql_transaction.execute(&format!("DROP TABLE IF EXISTS {};", table_name.0), [])?;
            }
            let table_scanned_blocks_name = scanned_nft_blocks_table_name()?;
            sql_transaction.execute(&format!("DROP TABLE IF EXISTS {};", table_scanned_blocks_name.0), [])?;
            sql_transaction.commit()?;
            Ok(())
        })
        .await
