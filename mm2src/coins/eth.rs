...
async fn withdraw_impl(coin: EthCoin, req: WithdrawRequest) -> WithdrawResult {
    ...
    let (tx_hash, tx_hex) = match coin.priv_key_policy {
        ...
        EthPrivKeyPolicy::Metamask(_) => {
            ...
            let tx_hash = coin.web3().await?.eth().send_transaction(tx_to_send).await?;
...
impl MarketCoinOps for EthCoin {
    ...
    fn send_raw_tx(&self, mut tx: &str) -> Box<dyn Future<Item = String, Error = String> + Send> {
        ...
        let fut = async move {
            let result = try_s!(coin.web3().await)
                .eth()
            ...
            loop {
                ...
                if let Some(event) = found {
                    if let Some(tx_hash) = event.transaction_hash {
                        let transaction = match try_tx_s!(selfi.web3().await)
                            .eth()
                            .transaction(TransactionId::Hash(tx_hash))
                            .await
                        {
...
async fn sign_and_send_transaction_with_metamask(
    ...
    // Please note that this method may take a long time
    // due to `wallet_switchEthereumChain` and `eth_sendTransaction` requests.
    let tx_hash = try_tx_s!(try_tx_s!(coin.web3().await).eth().send_transaction(tx_to_send).await);
...
impl EthCoin {
    ...
    async fn process_eth_history(&self, ctx: &MmArc) {
        ...
        loop {
            ...
            let web3 = match self.web3().await {
                Ok(t) => t,
                Err(e) => {
                    ctx.log.log(
                        "",
                        &[&"tx_history", &self.ticker],
                        &ERRL!("Couldn't connect to client. Error: {} - retrying..", e),
                    );
                    Timer::sleep(3.).await;
                    continue;
                },
            ...
            *self.history_sync_state.lock().unwrap() = HistorySyncState::InProgress(json!({
            ...
            for trace in saved_traces.traces {
                ...
                let receipt = match web3.eth().transaction_receipt(trace.transaction_hash.unwrap()).await {
                    Ok(r) => r,
                    Err(e) => {
                        ctx.log.log(
                            "",
        ...
        loop {
            ...
            *self.history_sync_state.lock().unwrap() = HistorySyncState::InProgress(json!({
            ...
            for event in all_events {
                ...
                let receipt = match web3.eth().transaction_receipt(event.transaction_hash.unwrap()).await {
                    Ok(r) => r,
                    Err(e) => {
                        ctx.log.log(
                            "",
...
impl EthCoin {
        ...
        loop {
            ...
            if let Some(event) = found {
                match event.transaction_hash {
                    Some(tx_hash) => {
                        let transaction = match try_s!(
                            try_s!(self.web3().await)
                                .eth()
                                .transaction(TransactionId::Hash(tx_hash))
                                .await
                        ) {
    ...
    pub fn get_gas_price(&self) -> Web3RpcFut<U256> {
        ...
        let fut = async move {
            ...
            let eth_gas_price = match coin.web3().await?.eth().gas_price().await {
            ...
            let fee_history_namespace: EthFeeHistoryNamespace<_> = coin.web3().await?.api();
    ...
    fn transaction_confirmed_at(&self, payment_hash: H256, wait_until: u64, check_every: f64) -> Web3RpcFut<U64> {
        ...
        let fut = async move {
            loop {
                ...
                let web3_receipt = match selfi.web3().await?.eth().transaction_receipt(payment_hash).await {
                    Ok(r) => r,
                    Err(e) => {
                        error!(
    ...
    fn wait_for_block(&self, block_number: U64, wait_until: u64, check_every: f64) -> Web3RpcFut<()> {
        ...
        let fut = async move {
            loop {
                ...
                match selfi.web3().await?.eth().block_number().await {
                    Ok(current_block) => {
