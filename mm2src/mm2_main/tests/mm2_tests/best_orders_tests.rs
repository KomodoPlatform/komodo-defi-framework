...
use mm2_test_helpers::for_tests::{best_orders_v2, best_orders_v2_by_number, eth_testnet_conf, get_passphrase,
                                  morty_conf, rick_conf, tbtc_conf, tbtc_segwit_conf, MarketMakerIt, Mm2TestConf,
                                  DOC_ELECTRUM_ADDRS, ETH_SEPOLIA_NODE, TBTC_ELECTRUMS};
...
fn test_best_orders_v2_exclude_mine() {
    ...
    let _ = block_on(enable_coins_eth_electrum(&mm_bob, ETH_SEPOLIA_NODE, None));
    let bob_orders = [
    ...
    let _ = block_on(enable_coins_eth_electrum(&mm_alice, ETH_SEPOLIA_NODE, None));
