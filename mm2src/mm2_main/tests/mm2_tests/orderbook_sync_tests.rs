...
use mm2_test_helpers::for_tests::{enable_z_coin_light, eth_testnet_conf, get_passphrase, morty_conf, orderbook_v2,
                                  ...
                                  ETH_SEPOLIA_NODE, MARTY_ELECTRUM_ADDRS, RICK, ZOMBIE_ELECTRUMS,
...
fn orderbook_should_work_without_coins_activation() {
    ...
    log!(
        "enable_coins (bob): {:?}",
        block_on(enable_coins_eth_electrum(&mm_bob, ETH_SEPOLIA_NODE, None))
    );
...
fn setprice_min_volume_should_be_displayed_in_orderbook() {
    ...
    log!(
        "enable_coins (alice): {:?}",
        block_on(enable_coins_eth_electrum(&mm_alice, ETH_SEPOLIA_NODE, None))
    );
