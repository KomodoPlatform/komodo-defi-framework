...
fn test_gui_storage() {
    ...
    let mm_new = block_on(MarketMakerIt::start_async(
        new_conf.conf,
        new_conf.rpc_password,
        new_conf.local,
    ))
    .unwrap();
    let (_bob_dump_log, _bob_dump_dashboard) = mm_new.mm_dump();
    log!("Log path: {}", mm_new.log_path.display());

    let resp = block_on(mm_new.rpc(&json!({
        "userpass": mm_new.userpass,
        "mmrpc": "2.0",
