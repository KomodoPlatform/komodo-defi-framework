...
async fn start_lightning(
    ...
    let platform_coin_address = platform_coin
        .my_address()
        .map_err(|e| EnableLightningError::Internal(format!("Error while getting platform coin address: {:?}", e)))?;
    // Initialize Persister
    let persister = init_persister(ctx, &platform_coin_address, conf.ticker.clone(), params.backup_path).await?;
    ...
    // Initialize DB
    let db = init_db(ctx, &platform_coin_address, conf.ticker.clone()).await?;
